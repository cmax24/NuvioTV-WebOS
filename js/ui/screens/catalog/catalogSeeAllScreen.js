...
export const CatalogSeeAllScreen = {
  ...
  render() {
    ...
    const title = descriptor.catalogName
      ? `${descriptor.catalogName} - ${I18n.formatContentType(descriptor.type)}`
      : I18n.t("catalog.fallback_title");
    const cards = this.items.length
      ? this.items.map((item) => `
          <article class="seeall-card focusable"
            ...
            <div class="seeall-card-title">${item.name || I18n.t("common.untitled")}</div>
            <div class="seeall-card-subtitle">${I18n.formatContentType(item.type || descriptor.type || "movie")}</div>
          </article>
        `).join("")
      : `<div class="seeall-empty">${I18n.t("catalog.no_items")}</div>`;

    this.container.innerHTML = `
      <div class="seeall-shell">
        <header class="seeall-header">
          <button class="seeall-back focusable" data-action="back">${I18n.t("common.back")}</button>
          ...
          <button class="seeall-load-more focusable${!this.hasMore || this.loading ? " disabled" : ""}"
            ...
            ${this.loading ? I18n.t("common.loading") : (this.hasMore ? I18n.t("catalog.load_more") : I18n.t("catalog.no_more"))}
          </button>
        </header>
