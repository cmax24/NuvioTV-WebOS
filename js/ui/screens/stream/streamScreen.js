...
function detectQuality(text = "") {
  ...
  return I18n.t("playback.quality_auto");
}
...
function flattenStreams(streamResult) {
  ...
  return (streamResult.data || []).flatMap((group) => {
    const groupName = group.addonName || I18n.t("search.default_addon");
    return (group.streams || []).map((stream, index) => ({
      id: `${groupName}-${index}-${stream.url || ""}`,
      label: stream.title || stream.name || I18n.t("stream.source_suffix", { addon: groupName }),
...
function mergeStreamItems(existing = [], incoming = []) {
  ...
  const push = (item) => {
    ...
    const key = [
      String(item.addonName || I18n.t("search.default_addon")),
      String(item.url || ""),
...
export const StreamScreen = {
  ...
  getHeaderMeta() {
    ...
    const title = String(this.params?.itemTitle || this.params?.playerTitle || I18n.t("common.untitled"));
  ...
  render() {
    ...
    const filterTabs = [
      `<button class="series-stream-filter focusable${this.addonFilter === "all" ? " selected" : ""}" data-action="setFilter" data-addon="all">${I18n.t("stream.filter_all")}</button>`,
      ...addons.map((addon) => `
    ...
    const streamCards = filtered.length
      ? filtered.map((stream) => `
          <article class="series-stream-card focusable" data-action="playStream" data-stream-id="${stream.id}">
            <div class="series-stream-title">${stream.label || I18n.t("stream.card_stream")}</div>
            <div class="series-stream-desc">${stream.description || ""}</div>
            <div class="series-stream-meta">
              <span>${stream.addonName || I18n.t("search.default_addon")}${stream.sourceType ? ` - ${stream.sourceType}` : ""}</span>
            </div>
            <div class="series-stream-tags">
              <span class="series-stream-tag">${detectQuality(stream.label || stream.description || "")}</span>
              <span class="series-stream-tag">${String(stream.sourceType || "").toLowerCase().includes("torrent") ? I18n.t("stream.tag_torrent") : I18n.t("stream.tag_stream")}</span>
            </div>
          </article>
        `).join("")
      : this.loading
        ? `<div class="series-stream-empty">${I18n.t("stream.loading_streams")}</div>`
        : `<div class="series-stream-empty">${I18n.t("stream.no_streams_filter")}</div>`;
      ...
      <div class="series-detail-shell stream-screen-shell">
        ...
        <div class="series-stream-panel stream-screen-panel">
          <div class="series-stream-left stream-screen-left">
            ...
            <div class="series-stream-episode-title">${subtitle || (isSeries ? I18n.t("stream.select_source_episode") : I18n.t("stream.select_source_movie"))}</div>
          </div>
  ...
  playStream(streamId) {
    ...
    Router.navigate("player", {
      ...
      episodeLabel: this.params?.season && this.params?.episode
        ...
        : null,
      playerTitle: this.params?.itemTitle || this.params?.playerTitle || I18n.t("common.untitled"),
