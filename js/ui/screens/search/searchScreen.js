  ...
  renderLoading() {
    this.container.innerHTML = `
      <div class="search-screen-shell">
        <div class="search-loading">${I18n.t("search.loading")}</div>
      </div>
    `;
  },
  ...
  async loadDiscoverRows() {
    ...
    return resolved
      ...
      .map((entry) => ({
        ...
        subtitle: I18n.t("search.from_addon", { addon: entry.addonName || I18n.t("search.default_addon") }),
        type: entry.type,
  ...
  async searchRows(query) {
    ...
    return responses
      ...
      .map(({ catalog, result }) => ({
        ...
        subtitle: I18n.t("search.from_addon", { addon: catalog.addonName || I18n.t("search.default_addon") }),
        type: catalog.type,
        addonBaseUrl: catalog.addonBaseUrl,
        addonId: catalog.addonId,
        addonName: catalog.addonName,
      ...
      if (this.mode === "search") {
        return `
          <div class="search-empty-state small">
            ...
            <h2>${I18n.t("search.no_results_title")}</h2>
            <p>${I18n.t("search.no_results_desc")}</p>
          </div>
        `;
      }
      return `
        <div class="search-empty-state">
          <img src="assets/icons/sidebar_search.svg" class="search-empty-icon" alt="" aria-hidden="true" />
          <h2>${I18n.t("search.start_title")}</h2>
          <p>${I18n.t("search.start_desc")}</p>
        </div>
      `;
    }
        ...
        <div class="search-results-track">
          ${(row.items || []).map((item) => `
            <article class="search-result-card focusable"
              ...
              <div class="search-result-poster-wrap">
                ${item.poster ? `<img class="search-result-poster" src="${item.poster}" alt="${item.name || I18n.t("common.content")}" />` : `<div class="search-result-poster placeholder"></div>`}
                ${watchedBadgeHtml(this.watchedIds, item.id)}
              </div>
              <div class="search-result-name">${item.name || I18n.t("common.untitled")}</div>
          ...
          <article class="search-result-card search-seeall-card focusable"
            ...
            <div class="search-seeall-inner">
              ...
              <div class="search-seeall-label">${I18n.t("search.see_all")}</div>
            </div>
          </article>
        </div>
      </section>
