...
const DEFAULT_GENRE_VALUE = "__default__";
...
function formatAddonTypeLabel(value) {
  ...
  if (!type) return I18n.t("type.movie");
  if (type === "tv") return I18n.t("type.tv");
  if (type === "series") return I18n.t("type.series");
  if (type === "movie") return I18n.t("type.movie");
...
export const DiscoverScreen = {
  ...
  async mount() {
    ...
    this.genreOptions = [DEFAULT_GENRE_VALUE];
    this.selectedGenre = DEFAULT_GENRE_VALUE;
    this.items = [];
    this.loading = true;
  ...
  updateGenreOptions() {
    ...
    this.genreOptions = [DEFAULT_GENRE_VALUE, ...genres];
  ...
  async reloadItems() {
    ...
    const extraArgs = {};
    if (this.selectedGenre && this.selectedGenre !== DEFAULT_GENRE_VALUE) {
  ...
  getPickerOptions(kind) {
    ...
    if (kind === "catalog") {
      return this.catalogOptions.map((entry) => ({
        value: entry.key,
        label: entry.catalogName || I18n.t("common.select")
      }));
    }
    if (kind === "genre") {
      return this.genreOptions.map((value) => ({
        value,
        label: value === DEFAULT_GENRE_VALUE ? I18n.t("discover.default_genre") : value
      }));
    }
    return [];
  },
  ...
  getCurrentPickerValue(kind) {
    ...
    if (kind === "genre") return this.selectedGenre || DEFAULT_GENRE_VALUE;
    return "";
  },
  ...
  setPickerValue(kind, value) {
    ...
    if (kind === "genre") {
      const safeValue = value || DEFAULT_GENRE_VALUE;
  ...
  render() {
    ...
    const title = selectedCatalog
      ? `${selectedCatalog.addonName || I18n.t("search.default_addon")} - ${formatAddonTypeLabel(selectedCatalog.type)}`
      : I18n.t("discover.no_catalog_selected");
    const cards = this.loading
      ? `<div class="discover-empty">${I18n.t("common.loading")}</div>`
      : (this.items.length
          ? this.items.map((item) => `
              <article class="discover-card focusable"
                       ...
                       data-item-title="${item.name || I18n.t("common.untitled")}">
                ...
                <div class="discover-card-title">${item.name || I18n.t("common.untitled")}</div>
              </article>
            `).join("")
          : `<div class="discover-empty">${I18n.t("discover.no_content")}</div>`);
    ...
    this.container.innerHTML = `
      <div class="discover-shell">
        ...
        <main class="discover-main">
          <h1 class="discover-title">${I18n.t("discover.title")}</h1>
          <section class="discover-filters">
            ${this.renderFilterPicker("type", I18n.t("discover.filter_type"), formatAddonTypeLabel(this.selectedType))}
            ${this.renderFilterPicker("catalog", I18n.t("discover.filter_catalog"), selectedCatalog?.catalogName || I18n.t("common.select"))}
            ${this.renderFilterPicker("genre", I18n.t("discover.filter_genre"), this.selectedGenre === DEFAULT_GENRE_VALUE ? I18n.t("discover.default_genre") : this.selectedGenre)}
          </section>
    ...
    this.container.addEventListener("click", (event) => {
      ...
      if (cardNode) {
        Router.navigate("detail", {
          ...
          fallbackTitle: cardNode.dataset.itemTitle || I18n.t("common.untitled")
        });
      }
    });
  },
