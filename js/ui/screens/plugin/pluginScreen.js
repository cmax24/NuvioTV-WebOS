      ...
      return `
        <article class="addons-installed-card">
          <div class="addons-installed-head">
            <div>
              <h3>${addon.displayName || addon.name || I18n.t("plugin.unknown_addon")}</h3>
            ...
            <div class="addons-installed-actions">
              <button class="addons-action-btn addons-focusable"
                      ...
                      data-row="${index + 3}"
                      data-col="0"
                      data-action-id="${upActionId}">${I18n.t("common.up")}</button>
              <button class="addons-action-btn addons-focusable"
                      data-zone="content"
                      data-row="${index + 3}"
                      data-col="1"
                      data-action-id="${downActionId}">${I18n.t("common.down")}</button>
              <button class="addons-action-btn addons-focusable addons-remove-btn"
                      data-zone="content"
                      data-row="${index + 3}"
                      data-col="2"
                      data-action-id="${removeActionId}">${I18n.t("common.remove")}</button>
            </div>
          </div>
          <p class="addons-installed-description">${addon.description || I18n.t("plugin.no_description")}</p>
        </article>
      `;
    }).join("");

    this.actionMap.set("install_addon", async () => {
      const value = window.prompt(I18n.t("plugin.prompt_install"), "https://example.com/manifest.json");
      if (value === null) {
        return;
      }
    ...
    this.container.innerHTML = `
      <div class="addons-shell">
        ...
        <main class="addons-main">
          <h1 class="addons-title">${I18n.t("plugin.title")}</h1>
          <section class="addons-install-card">
            <h2>${I18n.t("plugin.install_title")}</h2>
            <div class="addons-install-row">
              <div class="addons-install-input">${I18n.t("plugin.install_url_hint")}</div>
              <button class="addons-install-btn addons-focusable"
                      data-zone="content"
                      data-row="0"
                      data-col="0"
                      data-action-id="install_addon">${I18n.t("common.install")}</button>
            </div>
          </section>
          <button class="addons-large-row addons-focusable"
            ...
            <span>
              <strong>${I18n.t("plugin.manage_phone_title")}</strong>
              <small>${I18n.t("plugin.manage_phone_desc")}</small>
            </span>
            <span class="addons-large-row-tail">${I18n.t("common.open")}</span>
          </button>
          <button class="addons-large-row addons-focusable"
                  data-zone="content"
                  data-row="2"
            ...
            <span>
              <strong>${I18n.t("plugin.reorder_title")}</strong>
              <small>${I18n.t("plugin.reorder_desc")}</small>
            </span>
            <span class="addons-large-row-tail">${I18n.t("common.sort")}</span>
          </button>
          <h2 class="addons-subtitle">${I18n.t("plugin.installed_title")}</h2>
          <section class="addons-installed-list">
            ${addonRows || `<div class="addons-empty">${I18n.t("plugin.empty_installed")}</div>`}
          </section>
        </main>
      </div>
    `;
