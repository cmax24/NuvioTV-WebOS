...
export const LibraryScreen = {
  ...
  renderLoading() {
    this.container.innerHTML = `
      <div class="library-shell">
        <div class="library-loading">${I18n.t("library.loading")}</div>
      </div>
    `;
  },

  async loadData() {
    const [savedItems, progressItems, watchedIds] = await Promise.all([
  ...
  renderSavedCards() {
    ...
    if (!items.length) {
      return `<p class="home-empty">${I18n.t("library.no_saved_items")}</p>`;
    }
    return `
  ...
  renderProgressCards() {
    ...
    if (!items.length) {
      return `<p class="home-empty">${I18n.t("library.no_continue_items")}</p>`;
    }
    return `
      <div class="home-track">
        ${items.map((item) => {
          ...
          return `
            <article class="home-content-card home-progress-card focusable"
              ...
              <div class="home-progress-poster"${meta.poster ? ` style="background-image:url('${meta.poster}')"` : ""}>
                <span class="home-progress-left">${durationMin > 0 ? I18n.t("home.minutes_left", { value: remaining }) : I18n.t("home.continue")}</span>
                ${watchedBadgeHtml(this.watchedIds, item.contentId)}
              </div>
      ...
      <div class="home-shell library-shell">
        <aside class="home-sidebar expanded">
          ...
          <div class="home-nav-list">
            <button class="home-nav-item focusable" data-action="gotoHome"><span class="home-nav-icon-wrap">${navIconSvg("gotoHome")}</span><span class="home-nav-label">${I18n.t("nav.home")}</span></button>
            <button class="home-nav-item focusable" data-action="gotoSearch"><span class="home-nav-icon-wrap">${navIconSvg("gotoSearch")}</span><span class="home-nav-label">${I18n.t("nav.search")}</span></button>
            <button class="home-nav-item focusable" data-action="gotoLibrary"><span class="home-nav-icon-wrap">${navIconSvg("gotoLibrary")}</span><span class="home-nav-label">${I18n.t("nav.library")}</span></button>
            <button class="home-nav-item focusable" data-action="gotoPlugin"><span class="home-nav-icon-wrap">${navIconSvg("gotoPlugin")}</span><span class="home-nav-label">${I18n.t("nav.addons")}</span></button>
            <button class="home-nav-item focusable" data-action="gotoSettings"><span class="home-nav-icon-wrap">${navIconSvg("gotoSettings")}</span><span class="home-nav-label">${I18n.t("nav.settings")}</span></button>
          </div>
        ...
        <main class="home-main library-main">
          <section class="library-topbar">
            <h2 class="library-title">${I18n.t("library.title")}</h2>
            <div class="library-type-tabs">
              <button class="library-type-tab focusable${this.selectedType === "all" ? " selected" : ""}" data-action="setType" data-type="all">${I18n.t("library.tab_all")}</button>
              <button class="library-type-tab focusable${this.selectedType === "movie" ? " selected" : ""}" data-action="setType" data-type="movie">${I18n.t("library.tab_movie")}</button>
              <button class="library-type-tab focusable${this.selectedType === "series" ? " selected" : ""}" data-action="setType" data-type="series">${I18n.t("library.tab_series")}</button>
            </div>
          </section>

          <section class="home-row">
            <h3 class="home-row-title">${I18n.t("library.row_continue")}</h3>
            ${this.renderProgressCards()}
          </section>

          <section class="home-row">
            <h3 class="home-row-title">${I18n.t("library.row_saved")}</h3>
  ...
  onKeyDown(event) {
    ...
    if (action === "openDetail") {
      Router.navigate("detail", {
        ...
        fallbackTitle: current.dataset.itemTitle || I18n.t("common.untitled")
      });
    }
    if (action === "setType") {
