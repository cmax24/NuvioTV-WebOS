...
const EPISODE_COMPLETION_RATIO = 0.95;
...
function episodeWatchKey(season, episode) {
...
function isCompletedEpisodeProgress(progress = {}) {
  ...
  return (positionMs / durationMs) >= EPISODE_COMPLETION_RATIO;
}

function hasStartedWatchProgress(progress = {}) {
  ...
  if (String(progress.videoId || "").trim()) {
    return true;
  }
...
function buildEpisodeWatchedSet(contentId, watchedItems = [], progressItems = []) {
  ...
  (watchedItems || []).forEach((item) => {
    if (String(item?.contentId || "").trim() !== wantedContentId) {
      return;
    }
    const key = episodeWatchKey(item?.season, item?.episode);
    if (key) {
      watchedSet.add(key);
    }
  });
  ...
  (progressItems || []).forEach((item) => {
    ...
    if (!isCompletedEpisodeProgress(item)) {
      return;
    }
    const key = episodeWatchKey(item?.season, item?.episode);
    if (key) {
...
export const MetaDetailsScreen = {
  ...
  async mount(params = {}) {
    ...
    this.episodeWatchedSet = new Set();
    ...
    this.hasStartedSeries = false;
    this.primaryEpisodeToPlay = null;
  ...
  async loadDetail() {
    ...
    const [isSaved, progress, watchedItem, allProgress, watchedItems] = await Promise.all([
      ...
      watchedItemsRepository.getAll(5000)
    ]);
    ...
    this.episodeWatchedSet = buildEpisodeWatchedSet(itemId, watchedItems, allProgress);
    const isSeriesContent = itemType === "series" || itemType === "tv";
    this.hasStartedSeries = isSeriesContent && (
      hasStartedWatchProgress(progress)
      || this.episodeWatchedSet.size > 0
      || Boolean(watchedItem)
    );
    ...
    this.primaryEpisodeToPlay = this.computePrimaryEpisodeToPlay(progress);
  ...
  computePrimaryEpisodeToPlay(progress) {
    if (!this.episodes?.length) {
      return null;
    }
    ...
    if (videoId) {
      currentEpisode = this.episodes.find((entry) => String(entry.id || "") === videoId) || null;
    }

    if (!currentEpisode && Number.isFinite(season) && Number.isFinite(episode) && season > 0 && episode > 0) {
      currentEpisode = this.episodes.find((entry) => Number(entry.season || 0) === season && Number(entry.episode || 0) === episode) || null;
    }
    ...
    if (isCompletedEpisodeProgress(progress)) {
      const currentIndex = this.episodes.findIndex((entry) => entry.id === currentEpisode.id);
      return this.episodes[currentIndex + 1] || currentEpisode;
    }
  ...
  renderSeriesLayout(meta) {
    ...
    const showResumeLabel = Boolean(this.hasStartedSeries);
    const primaryPlayLabel = showResumeLabel ? I18n.t("detail.resume") : "";
    const primaryPlayClass = `series-primary-btn focusable${showResumeLabel ? "" : " icon-only"}`;
    const primaryPlayAriaLabel = showResumeLabel ? I18n.t("detail.resume") : I18n.t("detail.play");
    ...
    const countryLine = Array.isArray(meta.country)
      ...
      : (meta.country || "");
    const descriptionText = String(meta.description || "No description.").trim() || "No description.";
    const canToggleDescription = descriptionText.length > 160;
    const descriptionClass = `series-detail-description${this.descriptionExpanded ? " expanded" : ""}`;
    const descriptionToggleLabel = this.descriptionExpanded ? I18n.t("detail.collapse") : I18n.t("detail.expand");
    ...
    this.container.innerHTML = `
      <div class="series-detail-shell">
        ...
        <div class="series-detail-content">
          ...
          ${canToggleDescription ? `<button class="series-description-toggle focusable" data-action="toggleDescription" aria-expanded="${this.descriptionExpanded ? "true" : "false"}">${descriptionToggleLabel}</button>` : ""}
          <p class="series-detail-meta">${metaInfo}${imdbBadge}</p>
          ${countryLine ? `<p class="series-detail-support">${countryLine}</p>` : ""}
          <div class="series-detail-actions">
            <button class="${primaryPlayClass}" data-action="playDefault" aria-label="${primaryPlayAriaLabel}">
              <span class="series-btn-icon">${renderPlayGlyph()}</span>
              ${showResumeLabel ? `<span>${primaryPlayLabel}</span>` : ""}
            </button>
    ...
    ScreenUtils.setInitialFocus(this.container, ".series-detail-actions [data-action=\"playDefault\"].focusable");
  },
  renderDefaultLayout(meta, streamItems) {
    const isSeries = meta.type === "series" || meta.type === "tv";
    const primaryCtaLabel = isSeries
      ? (this.hasStartedSeries ? I18n.t("detail.resume") : I18n.t("detail.play"))
      : I18n.t("detail.play");
    ...
    this.container.innerHTML = `
      ...
      <div class="row">
        <div class="card focusable" data-action="playDefault">${primaryCtaLabel}</div>
    ...
    this.container.innerHTML = `
      <div class="series-detail-shell movie-detail-shell">
        ...
        <div class="series-detail-content movie-detail-content">
          ...
          ${countryLine ? `<p class="series-detail-support">${countryLine}</p>` : ""}
          <div class="series-detail-actions">
            <button class="series-primary-btn icon-only focusable" data-action="playDefault" aria-label="${I18n.t("detail.play")}">
              <span class="series-btn-icon">${renderPlayGlyph()}</span>
            </button>
            <button class="series-circle-btn focusable" data-action="toggleLibrary">
              ${this.isSavedInLibrary ? "-" : `<img class="series-btn-svg" src="assets/icons/library_add_plus.svg" alt="" aria-hidden="true" />`}
  ...
  renderEpisodeCards() {
    ...
    return selectedSeasonEpisodes.map((episode) => {
      const watchedKey = episodeWatchKey(episode.season, episode.episode);
      const isEpisodeWatched = Boolean(watchedKey && this.episodeWatchedSet?.has(watchedKey));
      return `
      <article class="series-episode-card focusable"
           data-action="openEpisodeStreams"
           data-video-id="${episode.id}">
        <div class="series-episode-thumb"${episode.thumbnail ? ` style="background-image:url('${episode.thumbnail}')"` : ""}>
          ${isEpisodeWatched ? `<span class="series-episode-watched" aria-label="Episode watched" title="Watched episode">&#10003;</span>` : ""}
        </div>
  ...
  handleSeriesDpad(event) {
    ...
    const descriptionToggle = this.container.querySelector(".series-description-toggle.focusable");
    ...
    if (descriptionToggle && current === descriptionToggle) {
      if (direction === "down") {
        if (actions.length) {
          return this.focusInList(actions, 0) || true;
        }
        if (seasons.length) {
    ...
    if (actionIndex >= 0) {
      ...
      if (direction === "up" && descriptionToggle) {
        return this.focusInList([descriptionToggle], 0) || true;
      }
      if (direction === "down") {
        if (seasons.length) {
  ...
  async onKeyDown(event) {
    ...
    if (action === "toggleDescription") {
      ...
      this.render(this.meta);
      ScreenUtils.setInitialFocus(this.container, ".series-description-toggle.focusable");
      return;
    }

    if (action === "playDefault") {
      if (this.params?.itemType === "series" || this.params?.itemType === "tv") {
        const targetEpisode = this.primaryEpisodeToPlay
    ...
    if (action === "playStream" && current.dataset.streamUrl) {
      const targetEpisode = this.primaryEpisodeToPlay || this.nextEpisodeToWatch || null;
      Router.navigate("player", {
        ...
        playerTitle: this.meta?.name || this.params?.fallbackTitle || this.params?.itemId || "Untitled",
        playerSubtitle: this.params?.itemType === "series" ? (targetEpisode?.title || "") : "",
        playerBackdropUrl: this.meta?.background || this.meta?.poster || null,
        playerLogoUrl: this.meta?.logo || null,
        episodes: this.episodes || [],
