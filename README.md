...
Secrets are no longer hardcoded in `js/config.js`.

1. Copy `js/config.runtime.example.js` to `js/config.runtime.js`
2. Set:
   - `SUPABASE_URL`
   - `SUPABASE_ANON_KEY`
   - `TV_LOGIN_REDIRECT_BASE_URL`

The app reads runtime values from `window.__NUVIO_CONFIG__` in `js/config.runtime.js`.
